...
/**
 * Customer "My Rentals" screen: styled read-only table of the logged-in
 * customer's rentals.
 */
public class MyRentalsPanel extends JPanel {
    ...
    public MyRentalsPanel(MainFrame parent) {
        this.parent = parent;
        ...
        model = new DefaultTableModel(
                new String[]{"ID", "Vehicle", "Start", "End",
                    "Total", "Status"}, 0) {
            @Override
    ...
    public void refresh() {
        ...
        for (Rental r : parent.getContext().rentalManager()
                .getCustomerRentals(user.getUserName())) {
            model.addRow(new Object[]{r.getRentalId(), r.getVehicle().getId(),
                r.getStartDate(), r.getEndDate(), r.getTotalPrice(), r.getStatus()});
        }
    }
}
